...
function decryptString(s) {
  ...
  console.log("Decrypted string:", sb);
  return sb;
}
...
// ---------------------------------------------------------
/**
 * @param {string} timestamp - Current timestamp (Date.now().toString())
 * @param {string} body - The request body string (utf8). Empty string if none. "{}" if multipart (check logic).
 * @param {string} deviceId - Header 'device-id' value
 * @param {string} androidId - Header 'android-id' value
 * @param {string} tn - Header 'tn' value
 */
function generateSn(timestamp, body, deviceId, androidId, tn) {
  ...
  // console.log("SignBaseString:", signBasedStr);
...
export { generateSn, decryptString };
